...
def local_PCA(site, num_PC, mean_removal=None, subject_level_PCA=True, subject_level_num_PC=120):
    ...
    for mm in subject_list:
        ...
        if subject_level_PCA:
            ...
            data_subject_tmp, projM[mm], bkprojM[mm] = base_PCA(   raw_subject,
                                                                   num_PC=subject_level_num_PC,
                                                                   axis=1,
                                                                   whitening=True)
            data_subject = np.hstack((data_subject, data_subject_tmp)) if data_subject.size else data_subject_tmp
        else:
            data_subject = np.hstack((data_subject, raw_subject)) if data_subject.size else raw_subject

    reduced_data, _, _ = base_PCA(data_subject,
                                  num_PC=num_PC,
                                  axis = 1,
                                  whitening=False)
...
def local_1(args):
    ...
    reduced_data, projM, bkprojM = local_PCA(Xs, num_PC=5*K, subject_level_PCA=True, subject_level_num_PC=80) 
